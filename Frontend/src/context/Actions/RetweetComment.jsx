...
const RetweetComment = async ({
  ...
  try {
    ...
    dispatchRetweet({
      type: ACTIONS.RETWEET_COMMENT_SUCCESS,
    ...
    setComment((prev) => {
      ...
      if (indexCommentInTempArray !== -1) {
        const indexOfUserInRetweetArray = tempComments[
          indexCommentInTempArray
        ].comment.retweets.findIndex((item) => {
          return item._id === state.user._id;
        });
        if (
          indexOfUserInRetweetArray !== -1 &&
          tempComments[indexCommentInTempArray].comment.retweets.length > 0
        ) {
          tempComments[indexCommentInTempArray].comment.retweets.splice(
            indexOfUserInRetweetArray,
            1
          );
          const parentId =
          ...
          if (parentIndex !== -1) {
            ...
            if (childIndex !== -1) {
              const indexOfUserInRetweetArray = tempComments[
                parentIndex
              ].comment.children[childIndex].retweets.findIndex((item) => {
                return item._id === state.user._id;
              });
              tempComments[parentIndex].comment.children[
                childIndex
              ].retweets.splice(indexOfUserInRetweetArray, 1);
            }
          }
        } else {
    ...
    dispatchRetweet({
      type: ACTIONS.RETWEET_COMMENT_FAILURE,
