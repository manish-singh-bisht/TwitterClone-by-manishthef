...
const MoreOptionMenuModalConversation = ({
  visibility,
  handleOutsideClick,
  buttonPosition,
  infoToMoreOptionModal,
  setMessages,
  setVisibilityMoreOptionModalConversation,
  ...
  const deleteForYou = async () => {
    ...
    await axios.delete(
      `${API_BASE_URL}/chat/conversation/deleteConversation/${infoToMoreOptionModal.conversationid}`,
      {
        data: {
          userId,
        },
  ...
  const pinHandler = async (handle, conversationid) => {
    if (!state.user.pinnedConversation) {
      const { data } = await axios.get(
        `${API_BASE_URL}/pinConversation/${handle}/${conversationid}`,
        {
          withCredentials: true,
          headers: {
            authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      state.user.pinnedConversation &&
      state.user.pinnedConversation !== infoToMoreOptionModal.conversationid
    ) {
      ...
      const { data } = await axios.get(
        `${API_BASE_URL}/unpinConversation/${handle}/${conversationid}`,
        {
          withCredentials: true,
          headers: {
            authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      setConversationsArray((prev) => {
        ...
        if (pinnedConversationIndex !== -1) {
          ...
          const originalIndex = prev.findIndex(
            (item) => item.createdAt < pinnedConversation.createdAt
          );
          ...
          <div className=" h-fit w-fit rounded-xl border-2 bg-white">
            <button
              ...
              onClick={() => {
                pinHandler(
                  state.user.handle,
                  infoToMoreOptionModal.conversationid
                );
            ...
            <button
              className="flex w-full items-center gap-3 rounded-b-xl p-3 text-black  hover:bg-gray-100"
              onClick={deleteForYou}
            >
              <Delete />
