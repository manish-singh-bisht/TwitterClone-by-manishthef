...
const MoreOptionMenuModalMessage = ({
  visibility,
  handleOutsideClick,
  buttonPosition,
  own,
  ...
  setMessageArray,
  messageFull,
  setVisibilityMoreOptionModalMessage,
  setReply,
}) => {
  ...
  const deleteForAll = async () => {
    ...
    const { data } = await axios.delete(
      `${API_BASE_URL}/chat/message/deleteForAll/${infoToMoreOptionModal.messageid}`,
      {
  ...
  const deleteForYou = async () => {
    ...
    const { data } = await axios.delete(
      `${API_BASE_URL}/chat/message/deleteMessage`,
      {
        data: {
          messageId,
          userId,
      ...
      <div className="fixed inset-0 z-30 h-[100vh] w-[100vw] ">
        ...
        <div className="relative z-30 ml-[5.2rem] w-fit md:ml-0" ref={modalRef}>
          <div className=" h-fit w-fit rounded-xl border-2 bg-white">
            ...
            {own && (
              <button
                className="flex w-full items-center gap-3 p-3 text-red-400  hover:bg-gray-100"
