...
const DraftModal = ({ visibilityDraft, handleOutsideClickDraft, closeAll }) => {
  ...
  const [drafts, setDrafts] = useState([
    { id: null, label: null, isChecked: null },
  ]);
    ...
    <>
      <div className="fixed inset-0 z-30 flex  h-[100vh] w-[100vw] items-center justify-center">
        <div
          ...
          onClick={handleOutsideClickDraft}
        ></div>
          ...
          <div>
            {drafts.length > 0 ? (
              drafts.map((item) => {
                return (
                  <div
                    className="flex h-fit gap-2 border-2 px-2 hover:bg-gray-100"
                    key={item.id}
                  >
                    <input
                      type="checkbox"
              ...
              <div className="mt-4 flex flex-col items-center justify-center  text-center text-[1.1rem] leading-4 text-gray-700">
                <span className="text-[1.6rem] font-bold text-black">
                  Hold that thought
                </span>{" "}
                <br /> Not ready to post just yet? Save it to your drafts.
              </div>
            )}
          </div>
