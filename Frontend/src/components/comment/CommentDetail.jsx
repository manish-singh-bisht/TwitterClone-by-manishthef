...
const CommentDetail = () => {
  ...
  useEffect(() => {
    ...
    const getCommentById = async () => {
      ...
      setParentCollection((prevCollection) => [
        ...prevCollection,
        data.comment,
      ]);
      setParentCollectionId((prevCollection) => [
        ...prevCollection,
        data.comment._id,
      ]);
    };

    const findIndexActiveCommentInComment = comment.comments.findIndex(
      (item) => {
    ...
    if (findIndexActiveCommentInComment !== -1) {
      if (
        comment.comments[findIndexActiveCommentInComment].comment.parent !==
          undefined &&
        !parentCollection.some(
          (item) =>
            item._id ===
            comment.comments[findIndexActiveCommentInComment].comment._id
        )
      ) {
        setParentCollection((prevCollection) => [
          ...prevCollection,
          comment.comments[findIndexActiveCommentInComment].comment,
        ]);
        ...
        if (parentCollectionId.includes(commentId)) {
          ...
          const updatedParentCollection = [...parentCollection].slice(
            0,
            index + 1
          );
          const updatedParentCollectionId = [...parentCollectionId].slice(
            0,
            index + 1
          );
      ...
      setComment((prev) => ({
        ...prev,
        activeComment: comment.comments[findIndexActiveCommentInComment],
      }));
    } else {
      getCommentById();
    }
      ...
      <main className="w-[100%] xl:grid xl:grid-cols-[44vw_auto]   ">
        <div className=" flex max-h-[full] min-h-[1400px] flex-col  border-l  border-r ">
          ...
          <Suspense fallback={<Loader />}>
            <ActiveComment
              dataActiveComment={comment.activeComment}
              commentId={commentId}
              postId={post._id}
              parent={commentId}
              ref={componentRef}
            ...
            {comment.activeComment.comment.children &&
              comment.activeComment.comment.children.length > 0 && (
                <CommentCard
