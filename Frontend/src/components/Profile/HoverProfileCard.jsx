    ...
    return (
        <>
            <div className="absolute z-30 ml-0 hidden w-[19.5rem] rounded-2xl border-2 bg-white p-2 drop-shadow-md md:block">
                    ...
                    {ownerId !== state.user._id && (
                        <button
                            className={`group w-fit rounded-full   ${
                                state.user.following.includes(ownerId) ? "border-2 bg-white text-black hover:border-red-200 hover:bg-red-100" : "bg-black text-white hover:text-gray-300 active:text-gray-400"
                ...
                <div className="mt-2 break-words px-2 text-base font-normal">{description}</div>
            </div>
        </>
    );
};
