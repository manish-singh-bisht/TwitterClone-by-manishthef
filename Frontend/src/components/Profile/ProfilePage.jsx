...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const getTweets = (userHandle) =>
      fetchDataForButton("Tweets", `${API_BASE_URL}/getTweets/${userHandle}`);

    const getProfileUser = async (userHandle) => {
      const { data } = await axios.get(`${API_BASE_URL}/user/${userHandle}`, {
        withCredentials: true,
        headers: {
          authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      ...
      setTotal(data.total);
      setIsPinned({
        bool: data.userProfile.pinnedTweet ? true : false,
        id: data.userProfile.pinnedTweet ? data.userProfile.pinnedTweet : null,
      });
    ...
    if (state.user.handle === userHandle) {
      ...
      setIsPinned({
        bool: state.user.pinnedTweet ? true : false,
        id: state.user.pinnedTweet ? state.user.pinnedTweet : null,
      });
      getTweets(userHandle);
    } else {
  ...
  const replyButtonHandler = () =>
    fetchDataForButton("Replies", `${API_BASE_URL}/getReply/${userHandle}`);
  const mediaButtonHandler = () =>
    fetchDataForButton(
      "Media",
      `${API_BASE_URL}/getPostWithMedia/${userHandle}`
    );
  const likedButtonHandler = () =>
    fetchDataForButton("Likes", `${API_BASE_URL}/getLikedPost/${userHandle}`);

  const url =
    (activeButton === "Tweets" &&
      `${API_BASE_URL}/getTweets/${userHandle}?page=`) ||
    (activeButton === "Replies" &&
      `${API_BASE_URL}/getReply/${userHandle}?page=`) ||
    (activeButton === "Media" &&
      `${API_BASE_URL}/getPostWithMedia/${userHandle}?page=`) ||
    (activeButton === "Likes" &&
      `${API_BASE_URL}/getLikedPost/${userHandle}?page=`);
        ...
        <>
          <main className="w-full xl:grid xl:grid-cols-[44vw_auto]   ">
            <div className="flex h-[100%] min-h-[1400px] flex-col  border-l  border-r">
                ...
                >
                  ...
                  {total > 0 ? (
                    <div className="text-[0.9rem] text-gray-500">
                      {total > 1 ? `${total} Tweets` : `${total}Tweet`}
                    </div>
                  ) : (
                    <div className="text-[0.9rem] text-gray-500">0 Tweets</div>
                  )}
                </div>
              </div>
              ...
              <div className="mt-5 flex h-fit w-full items-center border-b">
                <TabButton
                  label="Tweets"
                  active={activeButton === "Tweets"}
                  onClick={tweetButtonHandler}
                />
                <TabButton
                  label="Replies"
                  active={activeButton === "Replies"}
                      ...
                      dataArray.map((item) => {
                        if (activeButton === "Tweets") {
                          const post = item.originalPost
                            ? item.originalPost
                            : item;
                          const ownerRetweet = item.userRetweeted
                            ? item.userRetweeted
                            : null;
                          const ownerImage =
                            post.owner.profile &&
                            post.owner.profile.image &&
                            post.owner.profile.image.url
                              ? post.owner.profile.image.url
                              : null;
                          ...
                          return (
                            <Post
                              ...
                              isCommentRetweet={
                                item.originalPost?.comment ? true : false
                              }
                              likes={post.likes}
                        ...
                        if (activeButton === "Replies") {
                          ...
                          const ownerImage =
                            ...
                            post.owner.profile.image.url
                              ...
                              : null;
                          const imageInPost = post.images ? post.images : null;
                          const commentParentPost = item.originalPost?.tweet
                            ? null
                            : item.originalPost?.comment
                              ? item.originalPost.comment.post
                              : item.post;

                          return (
                            <div
                              key={`overall+${post.createdAt}+${post._id}+${commentParentPost?.createdAt}`}
                            >
                              {commentParentPost && (
                                <div key={commentParentPost._id}>
                                  ...
                                  <div className="relative  ">
                                    <Post
                                      ...
                                      postImage={
                                        commentParentPost.images
                                          ? commentParentPost.images
                                          : null
                                      }
                                      commentsChildren={
                                        commentParentPost.comments
                                      }
                                      ownerName={commentParentPost.owner.name}
                                      description={
                                        commentParentPost.owner.description
                                      }
                                      ownerImage={
                                        commentParentPost.owner.profile &&
                                        commentParentPost.owner.profile.image &&
                                        commentParentPost.owner.profile.image
                                          .url
                                          ? commentParentPost.owner.profile
                                              .image.url
                                      ...
                                      whoCanReply={
                                        commentParentPost.whoCanReply
                                      }
                                      whoCanReplyNumber={
                                        commentParentPost.whoCanReplyNumber
                                      }
                                    />
                              ...
                              <Post
                                ...
                                ownerRetweet={ownerRetweet}
                                isCommentReply={
                                  item.originalPost?.tweet ? false : true
                                }
                                likes={post.likes}
                                postImage={imageInPost}
                                retweets={post.retweets}
                        ...
                        if (
                          ...
                          return (
                            <Post
                              ...
                              timeCreated={post.createdAt}
                              handler={
                                post.comment ? CommentLikeUnlike : LikeUnlike
                              }
                              dispatch={
                                post.comment
                                  ? dispatchCommentLikeUnlike
                              ...
                              handlerRetweet={
                                post.comment ? RetweetComment : RetweetPost
                              }
                              handlerBookmark={
                                post.comment ? CommentBookmark : PostBookmark
                              }
                              dispatchBookmark={
                                post.comment
                                  ? dispatchBookmarkComment
                      ...
                      <div>
                        {dataArray.length === 0 && (
                          <div>
                            {activeButton === "Media" &&
                              renderEmptyMessage(
                                "hasn't Tweeted media",
                                user.handle
                              )}
                            {activeButton === "Likes" &&
                              renderEmptyMessage(
                                "hasn't liked any Tweets",
                                user.handle
                              )}
                            {activeButton === "Replies" &&
                              renderEmptyMessage(
                                "hasn't replied Tweets",
                                user.handle
                              )}
                            {activeButton === "Tweets" &&
                              renderEmptyMessage(
          ...
          <Suspense fallback={<Loader />}>
            <UpdateModal
              ...
              banner={
                user.profile && user.profile.banner && user.profile.banner.url
                  ? user.profile.banner.url
                  : null
              }
              name={user.name}
...
export const TabButton = ({ label, active, onClick }) => (
  <button
    className={`w-full  font-semibold hover:bg-gray-200 md:w-fit md:px-12 md:text-[1.05rem] ${!active ? "py-[1rem] text-gray-600" : "pt-[0.8rem] text-black"}`}
    disabled={active}
    onClick={onClick}
  >
    {label}
