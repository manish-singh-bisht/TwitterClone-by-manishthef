...
const ModalForLikesRetweets = React.lazy(
  () => import("../Modal/ModalForLikesRetweets")
);
...
const TweetDetail = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    let value;
    if (isThread) {
      const { data } = await axios.get(`${API_BASE_URL}/${postId}/thread`, {
  ...
  const likeHandler = async () => {
    ...
    await LikeUnlike({ dispatch, ACTIONS, postId, state, setComment });
    if (isLiked) {
      setLikedBy((prev) =>
        prev.filter((item) => {
  ...
  const [animationRetweet, retweetValue, handleRetweetAnimation] = useAnimation(
    isRetweet,
    setIsRetweet,
    retweet,
    setRetweet
