...
const BookMarkPage = () => {
  ...
  useEffect(() => {
    ...
    const functionToGetAllBookmarks = async () => {
      ...
      const { data } = await axios.get(
        `${API_BASE_URL}/getBookmarks/${state.user._id}`,
        {
  ...
  const deleteAllBookmarks = async () => {
    setBookmarks([]);
    await axios.delete(`${API_BASE_URL}/deleteAllBookmarks/${state.user._id}`, {
      withCredentials: true,
      headers: {
        authorization: `Bearer ${localStorage.getItem("token")}`,
      },
            ...
            {bookmarks.length > 0 ? (
              ...
              <div className="mt-[4.3rem] flex flex-col items-center justify-center">
                ...
                <div className="text-center text-gray-600">
                  Don't let the good ones fly away! Bookmark <br /> Tweets to
